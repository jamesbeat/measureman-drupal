...
function MeasuremanView(canvas) {
    "use strict";
	...
	// A fresh model.
	this.model_ = new MeasuremanModel();
	if (this.controller_ == undefined) {
	    this.controller_ = new MeasuremanController(canvas, this);
	}
    }
    ...
    this.drawAll = function() {
		...
		drawMeasureBox(this.measure_ctx_, box, this.print_factor_, this.show_deltas_,
				true);
	    }
		...
		this.model_.forAllBoxes(function(box) {
		    drawMeasureBox(ctx, box, length_factor, show_deltas, false);
		});
		...
		else if (this.model_.hasEditBox()) {
		    var box = this.model_.getEditBox();
		    drawEditBox(ctx, box, this.print_factor_);
		    if (this.show_deltas_) {
			drawDeltaBox(ctx, box, this.print_factor_);
		    }
		}
    ...
    function drawEditBox(ctx, box, length_factor) {
		var pixel_len = box.width();
	
		// We want to draw the line a little bit shorter, so that the
		// open crosshair cursor has 'free sight'
		var dx = box.p2.x - box.p1.x;
		var dy = box.p2.y - box.p1.y;
		if (pixel_len > 2) {
		    dx = dx * (pixel_len - 2)/pixel_len;
		    dy = dy * (pixel_len - 2)/pixel_len;
		}
    ...
    // General draw of a measuring line.
    function drawMeasureBox(ctx, box, length_factor, show_deltas, highlight) {
		var print_text = (box.width()+" x "+box.height()+"\n"+"x:"+Math.floor(box.p1.x)+"\n"+"y:"+Math.floor(box.p1.y));
		
		
		if (show_deltas && box.p1.x != box.p2.x && box.p1.y != box.p2.y) {
		    var dx = length_factor * (box.p2.x - box.p1.x);
		    var dy = length_factor * (box.p1.y - box.p2.y);
		    print_text += "; \u0394=(" + Math.abs(dx).toPrecision(4) + ", "
			+ Math.abs(dy).toPrecision(4) + ")";
		}
		ctx.beginPath();
		// Some contrast background.
		if (highlight) {
		    ctx.strokeStyle = background_highlight_box_style;
		} else {
		    ctx.strokeStyle = background_box_style;
		}
		ctx.lineWidth = background_box_width;
		ctx.lineCap = 'round';
		ctx.moveTo(box.p1.x, box.p1.y);
		ctx.lineTo(box.p2.x, box.p1.y);
		ctx.lineTo(box.p2.x, box.p2.y);
		...
		// actual line
		if (highlight) {
		    ctx.strokeStyle = highlight_box_style;
		} else {
		    ctx.strokeStyle = box_style;
		}
		ctx.lineWidth = 1;
		ctx.moveTo(box.p1.x, box.p1.y);
		ctx.lineTo(box.p2.x, box.p1.y);
		...
		// .. and text.
		writeLabel(ctx, print_text,
			   (box.p1.x + box.p2.x)/2, (box.p1.y + box.p2.y)/2, "center");
	}
